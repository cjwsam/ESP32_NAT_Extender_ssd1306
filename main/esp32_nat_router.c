/* esp32OLEDBatt NAT Router/Extender Beta
...
void printLnOLED(char* line)
{
	ssd1306_clear_screen(&dev, false);
	ssd1306_display_text(&dev, 2, "                " , 16, true);
	ssd1306_display_text(&dev, 3, line , 16, true);
	ssd1306_display_text(&dev, 4, "                " , 16, true);
...
void countDownOLED()
{
	...
	ssd1306_display_image(&dev, 5, (6*8+6), image, sizeof(image));
	ssd1306_display_image(&dev, 5+1, (6*8+6), image, sizeof(image));
	ssd1306_display_image(&dev, 5+2, (6*8+6), image, sizeof(image));
	for(int font=0x39;font>=0x30;font--) {
		memset(image, 0, sizeof(image));
		ssd1306_display_image(&dev, 5+1, (7*8+6), image, 8);
...
/*void printImageOLED()
{
	ssd1306_clear_screen(&dev, false);
	// Display Count Down
	ssd1306_display_image(&dev, 5, (6*8+6), logo, sizeof(logo));
	ssd1306_display_image(&dev, 5+1, (6*8+6), logo, sizeof(logo));
	ssd1306_display_image(&dev, 5+2, (6*8+6), logo, sizeof(logo));
		ssd1306_display_image(&dev, 5+1, (7*8+6), logo, 8);
...
void wifi_init(const char* ssid, const char* passwd, const char* ap_ssid, const char* ap_passwd)
{
    ...
    //////////////////////////////
        ...
        center = 3;
    	bottom = 8;
    	ssd1306_display_text(&dev, 0, "    BOOTING     ", 16, true);
    	ssd1306_display_text(&dev, 2, " Initialising...", 16, false);
    	ssd1306_display_text(&dev, 4, "WIFI Connection ", 16, false);
    ...
    ESP_ERROR_CHECK(esp_event_loop_init(wifi_event_handler, NULL) ); //DO NOT DELETE THIS LINE !
...
void initOLED()
{
	...
	ssd1306_display_text(&dev, 0, "    BOOTING     ", 16, true);
	ssd1306_display_text(&dev, 2, " Initialising...", 16, false);
	ssd1306_display_text(&dev, 4, "   i2c 128x64   ", 16, false);
	...
	bottom = 4;
	ssd1306_display_text(&dev, 0, "    BOOTING     ", 16, true);
	ssd1306_display_text(&dev, 1, " Initialising...", 16, false);
	ssd1306_display_text(&dev, 2, " SSD1306 128x32 ", 16, false);
#endif // CONFIG_SSD1306_128x32
